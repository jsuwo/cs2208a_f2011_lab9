...
define(`align_d', `ifelse($1,0,0,eval($1 < 0),1,
`eval($2 * ((($1 + 1) / $2) - 1))',
`eval($2 * ((($1 - 1) / $2) + 1))')')
`%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
define(`local_var', `!local variables
define(`last_sym', 0)')
define(`var', `define(`last_sym',align_d(
  eval(last_sym - ifelse($3,,$2,$3)), $2))$1 = last_sym')

define(`begin_main',`.global	main
main:	save	%sp, align_d( 
  eval( -92 ifdef(`last_sym',`+ last_sym')), 8), %sp')
...
$1,name_of_funct,`ret
	restore' `ifelse(
	$2,,,  `$2, $3, $4')'undefine(`name_of_funct'),`
...
`generate code to multiply in terms of shifts < and adds +'
define(translate_d,
...
$1,$3,`errprint(
	`cmul: multiplicand and temp registers must be different')',
index(0123456789, substr($2,0,1)),-1,`errprint(
`cmul: attempt to covert non numeric constant')',
`
			    !start open coded multiply for
			    !$4 = $1 * $2, using $3 as temp
	start_d(
	$1,compact_d(booth_d(translate_d(convert_d($2)))),$3,$4)
